...
using System;
    ...
    /// </summary>
    public class SQLiterDB {
        public static SQLiterDB Instance = null;
        ...
        protected static string _sqlDBLocation = "";
  ...
  //          //SQLiteInit();
		...
		/// </summary>
		public void RunAsyncInit(string SQL_DB_NAME)
		{
			LoomManager.Loom.QueueOnMainThread(() =>
			{
				SQLiteInit(SQL_DB_NAME);
			});
		}
        ...
        /// </summary>
        protected void SQLiteInit(string SQL_DB_NAME) {
        ...
        protected void PrepareTable(string SQL_TABLE_NAME, Dictionary<string, string> columns) {
  ...
  //      public void InsertWord(string name, string definition)
		...
		/// <param name="value"></param>
		/// <returns></returns>
		public string Query(string table, string columnToSelect, string columnToFind, string value)
		{
			string text = "Not Found";
			_connection.Open();
			_command.CommandText = "SELECT " + columnToSelect + " FROM " + table + " WHERE " + columnToFind + "='" + value + "'";
			_reader = _command.ExecuteReader();
			if (_reader.Read())
				text = _reader.GetString(0);
			else
				Debug.Log("Query string - nothing to read...");
			_reader.Close();
			_connection.Close();
			return text;
		}

        public int Query(string table, string columnToSelect, string columnToFind, int value) {
            int num = 0;
            _connection.Open();
            _command.CommandText = "SELECT " + columnToSelect + " FROM " + table + " WHERE " + columnToFind + "=" + value;
            _reader = _command.ExecuteReader();
            if (_reader.Read())
                ...
                Debug.Log("Query int - nothing to read...");
            _reader.Close();
            _connection.Close();
            return num;
        }

        public float Query(string table, string columnToSelect, string columnToFind, float value) {
            float num = 0f;
            _connection.Open();
            _command.CommandText = "SELECT " + columnToSelect + " FROM " + table + " WHERE " + columnToFind + "=" + value;
            _reader = _command.ExecuteReader();
            if (_reader.Read())
                num = _reader.GetFloat(0);
            else
                Debug.Log("Query float - nothing to read...");
            _reader.Close();
            _connection.Close();
            return num;
        }
        ...
        /// </summary>
        /// <param name="value"></param>
        public void SetValue(string table, string columnToSet, int valueToSet, string columnToFind, string valueToFind)
		{
			ExecuteNonQuery("UPDATE OR REPLACE " + table + " SET " + columnToSet + "=" + valueToSet + " WHERE " + columnToFind + "='" + valueToFind + "'");
		}

        public void SetValue(string table, string columnToSet, float valueToSet, string columnToFind, string valueToFind) {
            ExecuteNonQuery("UPDATE OR REPLACE " + table + " SET " + columnToSet + "=" + valueToSet + " WHERE " + columnToFind + "='" + valueToFind + "'");
        }

        public void SetValue(string table, string columnToSet, string valueToSet, string columnToFind, string valueToFind) {
            ExecuteNonQuery("UPDATE OR REPLACE " + table + " SET " + columnToSet + "='" + valueToSet + "' WHERE " + columnToFind + "='" + valueToFind + "'");
        }

        public void SetValue(string table, string columnToSet, string valueToSet, string columnToFind, int valueToFind) {
            ExecuteNonQuery("UPDATE OR REPLACE " + table + " SET " + columnToSet + "='" + valueToSet + "' WHERE " + columnToFind + "=" + valueToFind);
        }

        public void SetValue(string table, string columnToSet, string valueToSet, string columnToFind, float valueToFind) {
            ExecuteNonQuery("UPDATE OR REPLACE " + table + " SET " + columnToSet + "='" + valueToSet + "' WHERE " + columnToFind + "=" + valueToFind);
        }

        public void SetValue(string table, string columnToSet, float valueToSet, string columnToFind, float valueToFind) {
            ExecuteNonQuery("UPDATE OR REPLACE " + table + " SET " + columnToSet + "=" + valueToSet + " WHERE " + columnToFind + "=" + valueToFind);
        }

        public void SetValue(string table, string columnToSet, int valueToSet, string columnToFind, float valueToFind) {
            ExecuteNonQuery("UPDATE OR REPLACE " + table + " SET " + columnToSet + "=" + valueToSet + " WHERE " + columnToFind + "=" + valueToFind);
        }

        public void SetValue(string table, string columnToSet, int valueToSet, string columnToFind, int valueToFind) {
            ExecuteNonQuery("UPDATE OR REPLACE " + table + " SET " + columnToSet + "=" + valueToSet + " WHERE " + columnToFind + "=" + valueToFind);
        }

        public void SetValue(string table, string columnToSet, float valueToSet, string columnToFind, int valueToFind) {
            ExecuteNonQuery("UPDATE OR REPLACE " + table + " SET " + columnToSet + "=" + valueToSet + " WHERE " + columnToFind + "=" + valueToFind);
        }
        ...
        public void DeleteWord(string table, string column, string value)
		{
			ExecuteNonQuery("DELETE FROM " + table + " WHERE " + column + "='" + value + "'");
		}

        public void DeleteWord(string table, string column, int value) {
            ExecuteNonQuery("DELETE FROM " + table + " WHERE " + column + "=" + value);
        }

        public void DeleteWord(string table, string column, float value) {
            ExecuteNonQuery("DELETE FROM " + table + " WHERE " + column + "=" + value);
        }
